#!/usr/bin/env python3
"""
Minimal, notebook-style tutorial (script) for shared_code.fun_bootstrap.
...
def demo_basic_percentiles():
    ...
    point, lo, hi = bootstrap_percentiles(
        x, q=q, n_boot=2000, ci=95.0, seed=0, chunk=256
    )
    for qi, pt, l, h in zip(q, point, lo, hi, strict=False):
...
def demo_diff_percentiles():
    ...
    for qi, pt, l, h, sig in zip(
        res["q"], res["point"], res["lo"], res["hi"], res["sig"], strict=False
    ):
        print(
            f"q={int(qi):>2}: diff={pt: .3f}, 95% CI=({l: .3f}, {h: .3f}), inside_CI={bool(sig)}"
        )
...
def demo_groups():
    ...
    qa = bootstrap_groups_percentiles(
        per_animal, groups, q=q, n_boot=1000, ci=95.0, seed=2, chunk=128
    )
    ...
    boots_map = bootstrap_groups_boots(
        per_animal, groups, q=q, n_boot=1000, seed=3, chunk=128
    )
    ...
    xA = pool_per_animal(per_animal, groups[A])
    xB = pool_per_animal(per_animal, groups[B])
    point = np.percentile(xA, q_arr) - np.percentile(xB, q_arr)
    print("Pairwise diff CI (reuse):")
    for qi, pt, l, h in zip(q_arr, point, lo, hi, strict=False):
