...
# If True, show tails properly
Y_LOG = True
Y_LOG_FLOOR = 1e-3     # <-- better for tails than 1e-3

# If you want less clutter, don’t fill null CI
FILL_NULL_CI = True   # <-- strongly recommended

# PDF x-limits (None to see full [-1,1])
X_LIM = (-0.8, 0.8)
...
def _assert_ok_pdf(bins, pdf, name):
    # negative density is not OK
    if np.any(pdf[np.isfinite(pdf)] < -1e-8):
        raise ValueError(f"[BAD] {name}: negative density found.")
    widths = np.diff(bins)
    area = float(np.nansum(pdf * widths))
    if DEBUG_PRINT and not (0.3 <= area <= 1.7):
        print(f"[WARN] {name}: PDF area={area:.3f} (expected ~1; subsampling can shift it).")
...
def _plot_pdf(ax, x, s_obs, s_null, *, color, label):
    # observed
    ...
    if FILL_NULL_CI:
        ax.fill_between(x, s_null["pdf_lo"], s_null["pdf_hi"], color=color, alpha=ALPHA_BAND * 0.6, linewidth=0)
...
def _plot_q(ax, p, s_obs, s_null, *, color, label):
    ...
    if FILL_NULL_CI:
        ax.fill_between(p, s_null["q_lo"], s_null["q_hi"], color=color, alpha=ALPHA_BAND * 0.6, linewidth=0)
...
for k in REQ:
    if not any(s.startswith(f"{k}__") for s in d.files):
        raise KeyError(f"Missing condition '{k}' in FP6b artifact.")
...
if DEBUG_PRINT:
    ...
    for kk in ["null_all", "null_intra", "null_inter", "null_trimer", "null_tetramer"]:
        _print_tail_debug(p_grid, S[kk], kk)
...
_plot_pdf(ax, x, S["obs_all"], S["null_all"], color="C0", label="All")
...
_plot_pdf(ax, x, S["obs_intra"], S["null_intra"], color="C0", label="Intra")
_plot_pdf(ax, x, S["obs_inter"], S["null_inter"], color="C1", label="Inter")
...
# =========================
fig, ax = plt.subplots(figsize=(6.8, 4.8))
_plot_pdf(ax, x, S["obs_trimer"], S["null_trimer"], color="C2", label="Trimer")
_plot_pdf(ax, x, S["obs_tetramer"], S["null_tetramer"], color="C3", label="Tetramer")
...
# =========================
# FIG 7 — 2×2 PDFs for atomic conditions (if present)
# =========================
atomic = [
...
if all(any(s.startswith(f"{k}__") for s in d.files) for k, _, _, _ in atomic):
    ...
    for ax, (k_obs, k_null, title, col) in zip(axes, atomic):
        ...
        _plot_pdf(ax, x, so, sn, color=col, label=title.replace(" × ", "-"))
        ax.set_title(title)
        ax.set_xlabel("MC value")
        ax.set_ylabel("Density")
        _maybe_xlim(ax, X_LIM)
        ax.legend(frameon=False, fontsize=9)
        set_log_pdf_axis(ax)  # <-- apply to each subplot
...
for key, label, color in obs_it_tt:
    s = _load_series(d, key)
    ax.plot(x, s["pdf_obs"], lw=LW, color=color, ls="-", label=label)
...
for key, label, color in obs_it_tt:
    s = _load_series(d, key)
    ax.plot(p_grid, s["q_obs"], lw=LW, color=color, ls="-", label=label)
