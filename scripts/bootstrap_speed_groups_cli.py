...
def _process_window_and_return(
    ...
    list[dict[str, object]],
    dict[tuple[tuple[str, str], tuple[str, str]], dict],
]:
    ...
    per_animal = load_per_animal_from_npz(
        npz, tau_index=None if tau_index < 0 else tau_index
    )
    if (
        reuse_group_boots
        ...
        boots_map = _central_bootstrap_groups_boots(
            ...
            seed=seed,
            chunk=chunk,
            dtype=(np.float32 if boots_float32 else float),
        )
        ...
        for gk, idxs in groups_map.items():
            if gk == "__q__":
                continue
            pooled_g = (
                _central_pool_per_animal(per_animal, idxs)
                if _central_pool_per_animal is not None
                else np.concatenate(
                    [per_animal[i] for i in idxs if i < len(per_animal)]
                )
            )
    ...
    if plot and not no_quantiles_win:
        ...
        fig_path = (
            fig_root / f"quantiles_{_sanitize(region_disp)}_win{win}.{plot_format}"
        )
    ...
    for A, B in groups_to_compare:
        ...
        if (
            ...
            pooled_A = (
                _central_pool_per_animal(per_animal, groups_map[A])
                if _central_pool_per_animal is not None
                else np.concatenate(
                    [per_animal[i] for i in groups_map[A] if i < len(per_animal)]
                )
            )
            pooled_B = (
                _central_pool_per_animal(per_animal, groups_map[B])
                if _central_pool_per_animal is not None
                else np.concatenate(
                    [per_animal[i] for i in groups_map[B] if i < len(per_animal)]
                )
            )
            ...
            qd = {
                ...
                "hi": hi,
                "sig": sig,
                "n_x": int(pooled_A.size),
                "n_y": int(pooled_B.size),
            }
            ...
            if plot and not no_diffs_win:
                ax = plot_quantile_diffs(
                    qd, title=f"{region_disp} | win={win} | {A}-{B}"
                )
                a_str = _sanitize(A)
                b_str = _sanitize(B)
                fig_path = (
                    fig_root
                    / f"diffs_{_sanitize(region_disp)}_win{win}_{a_str}_vs_{b_str}.{plot_format}"
                )
                if not (load_cache and fig_path.exists()):
                    ax.figure.savefig(fig_path, dpi=150, bbox_inches="tight")
                plt.close(ax.figure)
...
def main():
    ap = argparse.ArgumentParser(
        description="Bootstrap speed percentiles per group/region/window."
    )
    ap.add_argument(
        "--tr", type=int, default=500, help="Select metadata by total_tr (e.g., 500)."
    )
    ap.add_argument(
        "--subset",
        type=str,
        default=None,
        help="Subset subfolder under speed/ (e.g., 'shared').",
    )
    ap.add_argument(
        "--tau-index",
        type=int,
        default=0,
        help="Tau index to select (None = pool all taus).",
    )
    ap.add_argument(
        "--group-cols",
        type=str,
    ...
    ap.add_argument(
        "--reuse-group-boots",
        action="store_true",
        help="Reuse per-group bootstrap replicates to compute all pairs (faster for many pairs).",
    )
    ap.add_argument(
        "--early-stop",
        type=float,
        default=0.0,
        help="Adaptive CI tolerance (fraction). 0 disables.",
    )
    ap.add_argument(
        "--seed", type=int, default=0, help="Base random seed for bootstrapping."
    )
    ap.add_argument(
        "--ci", type=float, default=95.0, help="Confidence level (percent)."
    )
    ap.add_argument(
        "--chunk",
        type=int,
        default=128,
        help="Bootstrap chunk size for vectorized resampling.",
    )
    ap.add_argument(
        "--boots-float32",
        action="store_true",
        help="Store bootstrap arrays in float32 to reduce memory.",
    )
    ap.add_argument(
        "--pool-threshold",
        type=str,
    ...
    ap.add_argument(
        "--pool-all",
        action="store_true",
        help="Also add an 'all' pool combining all windows.",
    )
    ap.add_argument(
        "--outdir",
        type=str,
        default=None,
        help="Output folder name under dataset paths; defaults to --subset or 'bootstrap'.",
    )
    ap.add_argument(
        "--plot",
        action="store_true",
        help="Save plots for per-group quantiles and pairwise diffs.",
    )
    ap.add_argument(
        "--plot-format",
        type=str,
    ...
    ap.add_argument(
        "--grid",
        action="store_true",
        help="Also save a grid figure aggregating all pairwise diffs per window/pool.",
    )
    ap.add_argument(
        "--grid-cols",
        type=int,
        default=2,
        help="Max columns in grid layout for pairwise diffs.",
    )
    ap.add_argument(
        "--no-diffs-win",
        action="store_true",
        help="Disable saving per-window pairwise diffs figures (diffs_*_win*.png).",
    )
    ap.add_argument(
        "--no-quantiles-win",
        action="store_true",
        help="Disable saving per-window group quantiles figures (quantiles_*_win*.png).",
    )
    ap.add_argument(
        "--plot-diffs-by-win",
        action="store_true",
        help="Save a summary plot per ROI and pair: diff(A-B) vs window with percentiles as legends (filled=significant, open=ns).",
    )
    ap.add_argument(
        "--plot-diffs-bywin-grid",
        action="store_true",
        help="Save a grid figure per ROI aggregating all by-window diff(A-B) vs window panels.",
    )
    ap.add_argument(
        "--bywin-grid-cols", type=int, default=2, help="Columns for bywin grid layout."
    )
    # Correlation with cognitive score (NOR)
    ap.add_argument(
        "--correlate-nor",
        action="store_true",
        help="Compute correlation between per-animal speed percentiles and NOR cognitive score.",
    )
    ap.add_argument(
        "--nor-col",
        type=str,
        default=None,
        help="Column name for NOR score in cognitive data; auto-detect if omitted.",
    )
    ap.add_argument(
        "--plot-only",
        action="store_true",
        help="Generate figures from existing CSVs in outdir; skip all bootstrap and NPZ processing.",
    )
    ap.add_argument(
        "--show-tau",
        action="store_true",
        help="Print tau range from metadata and exit.",
    )
    ap.add_argument(
        "--progress",
        action="store_true",
        help="Show progress bars for regions/windows/pools (requires tqdm).",
    )
    ap.add_argument(
        "--load-cache",
        action="store_true",
        help="Reuse existing outputs if present; skip recomputation/plots where possible.",
    )
    ap.add_argument(
        "--jobs",
        type=int,
        default=1,
        help="Parallel jobs for per-window processing (1 = serial).",
    )
    ap.add_argument(
        "--parallel-scope",
        type=str,
    ...
    ap.add_argument(
        "--print-args",
        action="store_true",
        help="Print effective arguments (including defaults applied) and continue.",
    )
    ap.add_argument(
        "--show-defaults",
        action="store_true",
        help="Print all default values and exit.",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
        help="List planned regions/windows, pools, and output paths; then exit without computing.",
    )
    ap.add_argument(
        "--list-inputs",
        action="store_true",
        help="Print the full list of NPZ files that will be read.",
    )
    ...
    if args.tau_index >= 0 and args.tau_index >= tau_count:
        raise ValueError(
            f"tau-index {args.tau_index} is out of range for tau_count={tau_count}. "
    ...
    if args.plot_only:
        if not q_path.exists() or not d_path.exists():
            raise FileNotFoundError(
                f"plot-only requires existing CSVs: {q_path} and {d_path}"
            )
        ...
        if fig_root is not None:
            ...
            by_roi_pair_q: dict[
                tuple[str, tuple[str, str]], dict[float, list[tuple[int, float, bool]]]
            ] = {}
            for r in diffs_rows:
                ...
                by_roi_pair_q.setdefault(key, {}).setdefault(qv, []).append(
                    (win, diffv, sig)
                )
            ...
            if args.plot_diffs_by_win and by_roi_pair_q:
                for (roi, (A, B)), qmap in by_roi_pair_q.items():
                    ...
                    color_for_q = {
                        float(qi): base_colors[i % len(base_colors)]
                        for i, qi in enumerate(qs_sorted)
                    }
                    for qi in qs_sorted:
                        ...
                        for xw, yw, s in zip(wins, diffs, sigs, strict=False):
                            ax.plot(
                                xw,
                                yw,
                                "o",
                                color=color,
                                mfc=(color if s else "none"),
                                mec=color,
                            )
                    ...
                    out_path = (
                        fig_root
                        / f"bywin_{_san(roi)}_{a_str}_vs_{b_str}.{args.plot_format}"
                    )
            ...
            if args.plot_diffs_bywin_grid and by_roi_pair_q:
                roi_to_pairs: dict[
                    ...
                    list[
                        tuple[
                            tuple[str, str], dict[float, list[tuple[int, float, bool]]]
                        ]
                    ],
                ] = {}
                ...
                for roi, items in roi_to_pairs.items():
                    ...
                    if handles:
                        fig.legend(
                            handles, labels, loc="upper right", title="Percentile"
                        )
                    fig.suptitle(f"{roi} | diff(A-B) vs window")
                    fig.tight_layout(rect=[0, 0, 0.98, 0.95])
                    grid_out = fig_root / f"bywin_grid_{_san(roi)}.{args.plot_format}"
    ...
    for region_dir in reg_iter:
        ...
        if args.jobs > 1 and args.parallel_scope == "windows":
            ...
            results = Parallel(n_jobs=args.jobs, prefer="processes")(
                delayed(_process_window_and_return)(
                    ...
                    args.ci,
                    args.seed,
                    float(args.early_stop or 0.0),
                    int(args.chunk),
                    bool(args.boots_float32),
                    bool(args.reuse_group_boots),
                    args.plot,
                    args.plot_format,
                    args.no_quantiles_win,
                    args.no_diffs_win,
                    fig_root,
                    args.load_cache,
                )
            ...
            for idx, (qr, dr, window_pairs_qd) in enumerate(results):
                ...
                if args.plot and args.grid and window_pairs_qd:
                    ...
                    try:
                        roi_name = (
                            _infer_region_from_filename(tasks[idx][1].name)
                            or region_label
                        )
                    except Exception:
                    ...
                    grid_path = (
                        fig_root
                        / f"grid_{_sanitize(roi_name)}_win{w}.{args.plot_format}"
                    )
            ...
            if args.progress and tqdm is not None:
                win_iter = tqdm(
                    win_files, desc=f"{region_label} windows", unit="win", leave=False
                )
            for win, npz in win_iter:
                ...
                if args.plot and args.grid and window_pairs_qd:
                    try:
                        region_name = (
                            _infer_region_from_filename(npz.name) or region_label
                        )
                    except Exception:
                        region_name = region_label
                    grid_path = (
                        fig_root
                        / f"grid_{_sanitize(region_name)}_win{win}.{args.plot_format}"
                    )
                    if not (args.load_cache and grid_path.exists()):
                        _plot_pairs_grid(
                            region_name,
        ...
        for roi_name, files in roi_map.items():
            ...
            if args.progress and tqdm is not None:
                pool_iter = tqdm(
                    pool_iter, desc=f"{roi_name} pools", unit="pool", leave=False
                )
            for pool_name, pool_windows in pool_iter:
                ...
                if (
                    args.reuse_group_boots
                    and _central_bootstrap_groups_boots is not None
                    and _central_ci_from_boots is not None
                ):
                    boots_map = _central_bootstrap_groups_boots(
                        pooled, groups_map, q=q, n_boot=args.n_boot, seed=args.seed
                    )
                    q_arr = boots_map.get("__q__", np.asarray(q, float))
                    qa = {}
                    for gk, idxs in groups_map.items():
                        if gk == "__q__":
                            continue
                        pooled_g = (
                            _central_pool_per_animal(pooled, idxs)
                            if _central_pool_per_animal is not None
                            else np.concatenate(
                                [pooled[i] for i in idxs if i < len(pooled)]
                            )
                        )
                        ...
                        for qi, pt, lo_i, hi_i in zip(
                            q_arr, point, lo, hi, strict=False
                        ):
                ...
                if args.correlate_nor:
                    ...
                    if nor_col is None:
                        cand_cols = [
                            c
                            for c in data.cog_data_filtered.columns
                            if "nor" in str(c).lower()
                        ]
                        if len(cand_cols) == 1:
                            nor_col = cand_cols[0]
                    ...
                    if (
                        nor_col is not None
                        and nor_col in data.cog_data_filtered.columns
                    ):
                        import numpy as _np
                        ...
                        for j, qi in enumerate(q_list):
                            ...
                            n_used = int(
                                _np.count_nonzero(
                                    _np.isfinite(mat[:, j]) & _np.isfinite(nor_vals)
                                )
                            )
                            corr_rows.append(
                                {
                ...
                if args.plot:
                    try:
                        ...
                        from speed_bootstrap_nb import (
                            plot_group_quantiles,  # type: ignore
                        )
                    ax = plot_group_quantiles(
                        qa, title=f"{roi_name} | pool={pool_name}"
                    )
                    fig_path = (
                        fig_root
                        / f"quantiles_{_sanitize(roi_name)}_pool-{pool_name}.{args.plot_format}"
                    )
                ...
                for A, B in groups_to_compare:
                    ...
                    if (
                        ...
                        pooled_A = (
                            _central_pool_per_animal(pooled, groups_map[A])
                            if _central_pool_per_animal is not None
                            else np.concatenate(
                                [pooled[i] for i in groups_map[A] if i < len(pooled)]
                            )
                        )
                        pooled_B = (
                            _central_pool_per_animal(pooled, groups_map[B])
                            if _central_pool_per_animal is not None
                            else np.concatenate(
                                [pooled[i] for i in groups_map[B] if i < len(pooled)]
                            )
                        )
                        point = np.percentile(pooled_A, q_arr) - np.percentile(
                            pooled_B, q_arr
                        )
                        sig = (lo > 0) | (hi < 0)
                        qd = {
                        ...
                        for qi, pt, lo_i, hi_i, s in zip(
                            q_arr, point, lo, hi, sig, strict=False
                        ):
                            diffs_rows.append(
                                {
                                    "region": roi_name,
                ...
                if args.plot and not args.no_diffs_win and args.grid and pool_pairs_qd:
                    grid_path = (
                        fig_root
                        / f"grid_{_sanitize(roi_name)}_pool-{pool_name}.{args.plot_format}"
                    )
                    if not (args.load_cache and grid_path.exists()):
                        _plot_pairs_grid(
                            roi_name,
    ...
    if fig_root is not None and args.plot and args.plot_diffs_by_win and diffs_rows:
        ...
        for r in diffs_rows:
            ...
            qv = float(r.get("q", 0.0))
            by_roi_pair_q.setdefault(key, {}).setdefault(qv, []).append(
                (win, float(r.get("diff", 0.0)), bool(r.get("significant", False)))
            )
        for (roi, (A, B)), qmap in by_roi_pair_q.items():
            ...
            for qi in qs_sorted:
                ...
                for x, y, s in zip(wins, diffs, sigs, strict=False):
                    if s:
            ...
            out_path = (
                fig_root
                / f"bywin_{_sanitize(roi)}_{a_str}_vs_{b_str}.{args.plot_format}"
            )
            fig.savefig(out_path, dpi=150, bbox_inches="tight")
            plt.close(fig)
        ...
        if args.plot_diffs_bywin_grid and by_roi_pair_q:
            ...
            roi_to_pairs: dict[
                str,
                list[
                    tuple[tuple[str, str], dict[float, list[tuple[int, float, bool]]]]
                ],
            ] = {}
            for (roi, pair), qmap in by_roi_pair_q.items():
                roi_to_pairs.setdefault(roi, []).append((pair, qmap))
