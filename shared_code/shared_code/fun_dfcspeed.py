...
# ===============================================================================
def _handle_dfc_speed_analysis(
    ts_data, window_size, lag, save_path, n_animals, nodes, **kwargs
):
    """
    ...
    if save_path:
        file_path = make_file_path(
            save_path / prefix, prefix, window_size, lag, n_animals, nodes
        )
        ...
        if file_path and file_path.exists():
            try:
                ...
                logger.warning(
                    f"Failed to load cached DFC speed (reason: {e}). Recomputing..."
                )
    else:
        file_path = None

    logger.info(
        f"Computing DFC speed (window_size={window_size}, lag={lag}, tau={tau}, method={method})"
    )

    # First, load DFC streams (following test version approach)
    dfc_file_path = make_file_path(
        save_path / "dfc", "dfc", window_size, lag, n_animals, nodes
    )
    ...
    if dfc_file_path and dfc_file_path.exists():
        try:
            ...
            dfc_stream = load_from_cache(
                dfc_file_path, key="dfc", logger=logger, label="dfc"
            )
        except Exception as e:
            logger.warning(
                f"Failed to load cached DFC stream (reason: {e}). Computing..."
            )

    # If no DFC stream loaded, we need to compute it or handle the error
    if dfc_stream is None:
        logger.error(
            "No DFC stream available. Cannot compute DFC speed without DFC data."
        )
    ...
    results = [
        dfc_speed(dfc_stream[i], vstep, method=method, return_fc2=True)
...
def handler_get_tenet(
    ...
    **kwargs,
    ...
    if prefix == "speed":
        return _handle_dfc_speed_analysis(
            ts_data, window_size, lag, save_path, n_animals, nodes, **kwargs
        )
    ...
    results = np.array(
        [
            ts2dfc_stream(ts_data[i], window_size, lag, format_data)
    ...
    **kwargs,
    ...
    try:
        ...
        valid_prefixes = [
            ...
            "mc",
            "speed",
        ]  # Added 'speed' for backward compatibility
        ...
        if not save_path:
            raise ValueError(
                f"Invalid prefix '{prefix}'. Save path not found in paths dictionary."
            )
...
def dfc_speed(
    dfc_stream, vstep=1, method="pearson", return_fc2=False, tril_indices=None
):
    """
...
def dfc_speed_oversampled_series(
    ...
    time_windows_range = np.arange(
        time_windows_min, time_windows_max + 1, time_window_step
    )
    ...
    for idx_tt, ws in tqdm(enumerate(time_windows_range)):
        ...
        speed_oversampl = np.array(
            [
                dfc_speed(aux_dfc_stream, vstep=windows_size + sp)[1][:height_stripe]
                for sp in tau_array
            ]
        )
...
def parallel_dfc_speed_oversampled_series(
    ts,
    window_parameter,
    lag=1,
    tau=3,
    ...
    def compute_speed_for_window_size(ws, prefix):
        ...
        speed_oversampl = [
            dfc_speed(aux_dfc_stream, vstep=int(ws + sp))[1][:height_stripe]
            for sp in tau_array
        ]
        return np.median(speed_oversampl, axis=1), (
            speed_oversampl if get_speed_dist else None
        )
...
def get_population_wpooling(wp_list, index_group):
    ...
    index_mask = (
        index_group.to_numpy()
        if hasattr(index_group, "to_numpy")
        else np.asarray(index_group)
    )
...
# %%
def check_and_rerun_missing_files(
    paths, prefix, time_window_range, lag, n_animals, roi, processors=1
):
    """
    ...
    missing_files = get_missing_files(
        paths, prefix, time_window_range, lag, n_animals, roi
    )
