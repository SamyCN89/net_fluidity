...
1) Canonical bundles → DFC streams
   ...
   - `scripts/speed/dfc_speed_compute.py` consumes those dFC bundles and generates per-window speed artefacts under `results/<dataset>/speed/`, preserving the legacy subset naming when `--region-subdir` is enabled. Supports ROI filtering via `--region-labels` / `--region-indices` and can iterate per ROI with `--per-region`. See `docs/speed_compute.md` for detailed usage.
