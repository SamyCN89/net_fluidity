...
time_window_range = np.arange(
    window_parameter[0], window_parameter[1] + 1, window_parameter[2]
)
...
for window_size in time_window_range:
    ...
    file_path = make_file_path(
        paths["dfc"], prefix, window_size, lag, n_animals, regions
    )
...
for window_size in time_window_range:
    ...
    prefix = "speed"
    file_path = make_file_path(
        paths["speed"], prefix, window_size, tau, n_animals, regions
    )
...
speeds_all_T = [
    [speeds_all[win][animal] for win in range(n_windows)] for animal in range(n_animals)
]
...
speeds_by_animal_tau = [
    [
        [speeds_all[win][animal * n_tau + tau_idx] for tau_idx in range(n_tau)]
