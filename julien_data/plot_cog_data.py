...
annotator = Annotator(
    ax, pairs, data=cog_data_filtered, x="genotype", y="index_NOR", hue="treatment"
)
...
plt.title(
    "NOR values by Genotype and Treatment\n(Mann-Whitney U test, p-values < 0.05)",
    fontsize=15,
)
...
(
    plt.savefig(
        paths["f_cog"] / "NOR" / "cog_data_violinplot_Mann_Whitney.png",
        dpi=300,
        bbox_inches="tight",
    )
    if savefig
...
(
    plt.savefig(
        paths["f_cog"] / "NOR" / "cog_data_violinplot_Mann_Whitney.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    if savefig
...
cog_data_filtered["group"] = (
    cog_data_filtered["genotype"] + "_" + cog_data_filtered["treatment"]
)
...
group_values = [
    cog_data_filtered[cog_data_filtered["group"] == g]["index_NOR"].values
    for g in groups
]
...
(
    plt.savefig(
        paths["f_cog"] / "NOR" / "cog_data_violinplot_Kruskal_Wallis.png",
        dpi=300,
        bbox_inches="tight",
    )
    if savefig
...
(
    plt.savefig(
        paths["f_cog"] / "NOR" / "cog_data_violinplot_Kruskal_Wallis.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    if savefig
...
grouped = (
    filtered_df.groupby(["genotype", "treatment"])["index_NOR"]
    .agg(["mean", "sem", "count"])
    .reset_index()
)
print(grouped)
...
all_speeds = np.concatenate(
    [speed for animal in speeds_all_T for speed in animal]
).astype(np.float32)
...
for animal_speeds in speeds_all_T:
    ...
    animal_pool = np.concatenate(
        [
            speed_arr[~np.isnan(speed_arr)]
            for speed_arr in animal_speeds
...
plt.figure(figsize=(8, 5))
plt.hist(
    all_speeds, bins=175, color="skyblue", histtype="step", linewidth=1.5, density=True
)
plt.xlabel("Speed")
plt.ylabel("Count")
...
for group, animal_idxs in groups.items():
    ...
    for animal_idx in animal_idxs:
        ...
        animal_tau_speeds = [
            ...
            for speed_arr in speeds_all_T[animal_idx]
            if speed_arr is not None
        ]
...
for idx, (group, animal_idxs) in enumerate(groups.items()):
    ...
    plt.axvline(
        median, color=color, linestyle="-", linewidth=1, label=f"{group} median"
    )
    plt.axvline(
        q05, color=color, linestyle="--", linewidth=1, label=f"{group} q=0.05/0.95"
    )
...
for idx, (group, animal_idxs) in enumerate(groups.items()):
    ...
    # Vertical lines
    plt.axvline(
        median, color=color, linestyle="-", linewidth=1.2, label=f"{group} median"
    )
...
for idx, (group, animal_idxs) in enumerate(groups.items()):
    ...
    color = palette[idx]
    plt.plot(
        window_sizes, medians_per_window, marker=".", label=str(group), color=color
    )
    plt.fill_between(
...
for win_idx in range(n_windows):
    ...
    if speeds_this_window:
        flat_speeds = np.concatenate(speeds_this_window)
        flat_speeds = flat_speeds[~np.isnan(flat_speeds)]
        speed_matrix[:, win_idx] = [
            np.quantile(flat_speeds, q) for q in quantile_levels
        ]
    else:
...
plt.clim(
    np.nanmin(np.log(speed_matrix)), np.nanmax(np.log(speed_matrix))
...
im = plt.imshow(
    ...
    norm=mcolors.LogNorm(
        vmin=np.nanmin(speed_matrix[speed_matrix > 0]), vmax=np.nanmax(speed_matrix)
    ),
)
...
fig.colorbar(
    im, ax=axes, orientation="vertical", fraction=0.02, pad=0.04, label="DFC Speed"
)
plt.tight_layout()
plt.show()
