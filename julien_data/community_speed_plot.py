...
window_file_total = (
    save_path
    / f"{prefix}_windows{len(time_window_range)}_tau{len(tau_range)}_animals_{n_animals}.pkl"
)
# %%
...
def pooled_community_speeds(community, animal_idxs, window_idx_list):
    ...
    for animal_idx in animal_idxs:
        ...
        for win_idx in window_idx_list:
            win_arr = communities_speed[community][
                win_idx
            ]  # shape: (n_animals, n_tau, n_timepoints)
...
for community in range(n_communities):
    for group, animal_idxs in groups.items():
        ...
        group_arr = (
            np.concatenate([a for a in arrs if a.size > 0]) if arrs else np.array([])
        )
...
for idx, (group_name, animal_indices) in enumerate(data.groups.items()):
    ...
    if pooled_speeds:
        ...
        if group_speeds.size > 0:
            ...
            sns.kdeplot(
                group_speeds, bw_adjust=0.5, label=label, color=color, linewidth=2.5
            )
...
ax3.set_title(
    f"Diff: {group_names[0][0]}-{group_names[0][1]} - {group_names[1][0]}-{group_names[1][1]}"
)
...
ax6.set_title(
    f"Diff: {group_names[2][0]}-{group_names[2][1]} - {group_names[3][0]}-{group_names[3][1]}"
)
...
ax7.set_title(
    f"Diff: {group_names[0][0]}-{group_names[0][1]} - {group_names[2][0]}-{group_names[2][1]}"
)
...
ax8.set_title(
    f"Diff: {group_names[1][0]}-{group_names[1][1]} - {group_names[3][0]}-{group_names[3][1]}"
)
...
ax9.set_title(
    f"Diff: {group_names[0][0]}-{group_names[0][1]} - {group_names[3][0]}-{group_names[3][1]}"
)
...
for i, group in enumerate(groups):
    ...
    if np.sum(mask) > 2:
        ...
        plt.plot(
            xfit, yfit, color=group2color[group], linestyle="-", linewidth=2, alpha=0.75
        )
...
for idx, (group, animal_indices) in enumerate(group_dict.items()):
    ...
    sig_idx = np.where((pvalues < 0.05) & ~np.isnan(pvalues) & ~np.isnan(correlations))[
        0
    ]
...
for pool_idx, idxs in enumerate(pools):
    for animal_idx in range(n_animals):
        ...
        if pooled_speeds:
            ...
            per_animal_summaries[pool_labels[pool_idx]].append(
                np.median(all_pooled)
            )  # Use median, or mean, or quantile
        else:
...
for panel_idx, (ax, (g1, g2)) in enumerate(
    ...
    ax.set_ylabel(f"Quantiles: {lab2}", fontsize=15, fontweight="bold")
    ax.set_title(
        f"Quantile–Quantile: {lab2} vs {lab1}", fontsize=15, fontweight="bold", pad=13
    )
    ax.set_xlim(global_min, global_max)
    ax.set_ylim(global_min, global_max)
    ax.tick_params(axis="both", labelsize=15, width=1.2)
