...
for c in np.unique(communities):
    ...
    comm_file = (
        save_path
        / f"{prefix}_windows{len(time_window_range)}_tau{len(tau_range)}_animals_{n_animals}_regions_{n_regions}.pkl"
    )
    print(comm_file)
...
# %%
for community in range(n_communities):
    for win_idx_list, pool_label in [
        (window_idx_all, "All windows"),
        (window_idx_short, "Short"),
        (window_idx_long, "Long"),
    ]:
...
def plot_community_histograms(
    communities_speed,
    groups,
    window_idx_list,
    pool_label="All windows",
    bins=60,
    kind="hist",
    kde=True,
    alpha=0.55,
...
plot_community_histograms(
    communities_speed, data.groups, window_idx_all, pool_label="All windows"
)
