...
for i, fc_mat in enumerate(tqdm(fc_wt_veh)):
    allegiance_mat[i], gamma_qmod_val[i], gamma_agreement_mat[i], communities_mat[i] = (
...
best_gamma_idx, consensus_partition, allegiance = consensus_pipeline(
    communities_mat, gammas
)
...
def compute_correlation(allegiance_matrices: np.ndarray) -> np.ndarray:
    ...
    for i in range(n_matrices):
        for j in range(i, n_matrices):
            corr, _ = spearmanr(
                allegiance_matrices[i].flatten(), allegiance_matrices[j].flatten()
            )
...
plt.xticks(
    ticks=np.arange(len(data.region_labels_preprocessed)),
    labels=data.region_labels_preprocessed,
    rotation=90,
)
plt.yticks(
    ticks=np.arange(len(data.region_labels_preprocessed)),
...
fcd = np.array(
    [dfc_stream2fcd(ts2dfc_stream(ts1, window_size=20, lag=1)) for ts1 in tqdm(data.ts)]
)
# %%
# plot fcd
plt.figure(figsize=(10, 8))
