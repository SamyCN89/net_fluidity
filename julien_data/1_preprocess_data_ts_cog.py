...
def main(filter_mode="exclude_shortest"):
    ...
    if filter_mode == "exclude_shortest":
        ...
        print(
            f"Loaded {len(ts_list)} time series with more than {min_timepoints} time points each."
        )
    ...
    # Print excluded mice
    excluded_ts_ids = [
        mid for mid in ts_ids if mid not in cog_data_filtered["mouse"].values
    ]
    excluded_cog_ids = [
        mid
        for mid in cog_data_filtered["mouse"].astype(str).values
        if mid not in ts_ids
    ]

    print(
        "Mice with time series but NO cognitive data:",
        excluded_ts_ids,
        (set(ts_ids) - set(cog_data_filtered["mouse"])),
    )
    print(
        "Mice with cognitive data but NO time series:",
        excluded_cog_ids,
        (set(cog_data_filtered["mouse"]) - set(ts_ids)),
    )
    ...
    with open(
        paths["preprocessed"]
        / f"metadata_animals_{n_animals}_regions_{regions}_tr_{total_tr}.pkl",
        "wb",
    ) as f:
    ...
    if all(ts.shape == ts_filtered[0].shape for ts in ts_filtered):
        ...
        np.savez(
            paths["preprocessed"]
            / f"ts_filtered_animals_{n_animals}_regions_{regions}_tr_{total_tr}.npz",
            ts=ts_array,
            n_animals=n_animals,
            total_tr=total_tr,
            regions=regions,
        ...
        print(
            f"Saved: ts_filtered_animals_{n_animals}_regions_{regions}_tr_{total_tr}.npz with shape {ts_array.shape}"
        )
    else:
        np.savez(
            paths["preprocessed"]
        ...
        print(
            f"Saved: ts_filtered_animals_{n_animals}_regions_{regions}_tr_{total_tr}.npz"
        )
    ...
    print(
        f"Saved: cog_data_filtered_animals_{n_animals}_regions_{regions}_tr_{total_tr}.csv"
    )
