...
ROOT = (
    Path("/media/samy/Elements2/Proyectos/LauraHarsan/dataset/julien/")
...
vel_label = (
    "%s-%ss (short)" % (aux_timewr[0], aux_timewr[limits[0]]),
...
def plot_wpool(*wp_vars, labels=None, name_data="all"):
    ...
    assert len(wp_vars) == len(
        labels
    ), "Each data group must have a corresponding label"
...
plot_wpool(
    wp_wt_veh, wp_mut_veh, labels=["WT VEH", "Dp1Yey VEH"], name_data="wt_LCTB92_veh"
)
plot_wpool(
    wp_wt_treat,
    wp_mut_treat,
    labels=["WT Treat", "Dp1Yey Treat"],
    name_data="wt_LCTB92_treat",
)
plot_wpool(
    wp_wt_treat,
    wp_wt_veh,
    wp_mut_treat,
    wp_mut_veh,
    labels=["WT Treat", "WT VEH", "Dp1Yey Treat", "Dp1Yey VEH"],
    name_data="multi_group",
)
...
bin_centers, norm_counts_list = compute_normalized_histograms(
    np.concatenate(wp_wt), np.concatenate(wp_mut)
)
# %%
# Plot
...
np.savez(
    SPEED_DIR / f"windows_pooling_{HASH_TAG}.npz",
    wpool_wt=wp_wt,
    wpool_mut=wp_mut,
)
...
def qq_diff(qq_data):
    qq_data = np.transpose(
        qq_data, (1, 0, 2)
    )  # Shape: (n_groups, n_wpools, n_quantiles)
...
wp_genotype_veh_treat = (
    wp_wt_veh,
    wp_wt_treat,
    wp_mut_veh,
    wp_mut_treat,
)  # window pooling for genotype
...
for vv in range(np.shape(slope_qq_gen)[0]):
    plt.plot(
        np.arange(len(np.diff(q_range))) / len(np.diff(q_range)),
        slope_qq_gen[vv],
        ".-",
        label=label_vel[vv],
    )
...
# # Parameters
n_replicas = 50
qq_gen_bootstrap = np.array(
    handler_bootstrap_permutation(wp_genotype, q_range, replicas=n_replicas)
)
...
def plot_bootstrap_ci_comparison(
    ...
    qq_bootstrap,
    group_labels=("WT", "Mut"),
    vel_labels=("short", "mid", "long"),
    title="Bootstrap CI per Window",
):
    """
    ...
    for i in range(n_windows):
        ...
        for g, label in enumerate(group_labels):
            ...
            plt.fill_between(
                q_range, low, high, color=f"C{g}", alpha=0.2, label=f"{label} CI"
            )
...
plot_bootstrap_ci_comparison(
    q_range, qq_data, qq_gen_bootstrap, group_labels=("WT", "Mut")
)
