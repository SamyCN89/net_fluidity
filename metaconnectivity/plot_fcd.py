...
def plot_fcdxsubjects(fcd, mouse_hash_cog, gen_label):
    ...
    # for idx_mice in range(2):
        ...
        plt.title(f" FCD (window={windows_size}, lag={lag}) mouse {mouse_hash_cog[idx_mice]} {gen_label[idx_mice]} {sex_label[idx_mice]}",
                y=1.05, loc='left', fontsize=13)
        ...
        plt.subplot(211)
        plt.title('2m')
        plt.imshow(fcd[idx_mice], aspect='auto', interpolation='none', cmap='RdBu_r', vmin=-1, vmax=1)
        # plt.colorbar()
        ...
        cbar.set_ticklabels(['-1', '-0.5', '0', '0.5', '1'], fontsize=12)  # optional labels
        ...
        plt.subplot(212)
        plt.title('4m')
        plt.imshow(fcd[idx_mice*2], aspect='auto', interpolation='none', cmap='RdBu_r', vmin=-1, vmax=1)
        plt.xticks([])
        plt.yticks([])
        plt.xlabel(r"$tw_{i}$")
        plt.ylabel(r"$tw_{j}$")
        ...
        plt.tight_layout()
        if save_fig:
            plt.savefig(paths["f_dfc"] / f"fcd_window_{windows_size}_lag_{lag}_mouse_#{mouse_hash_cog[idx_mice]}_{gen_label[idx_mice]}_{sex_label[idx_mice]}.pdf")
            plt.savefig(paths["f_dfc"] / f"fcd_window_{windows_size}_lag_{lag}_mouse_#{mouse_hash_cog[idx_mice]}_{gen_label[idx_mice]}_{sex_label[idx_mice]}.png")
        plt.close()
...
for windows_size in windows_size_range:
    ...
    dfc_stream = np.array([ts2dfc_stream(ts[aa], windows_size, lag, format_data='2D')
