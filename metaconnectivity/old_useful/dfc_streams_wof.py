...
# Compute speed by window oversampling and fcd for one window in one animal
# =============================================================================
idx_mice = 1
...
#Window oversampling
speed_oversampl    = np.array([dfc_speed(dfc_streamaux, vstep=windows_size + sp)[1][:height_stripe] for sp in tau_array])#get the stripe of speeds around W

fcd             = dfc_stream2fcd(dfc_streamaux)
#%%
idx_mice=20
#Fcd and dfc speed stream plot
plt.figure(4)
plt.clf()
plt.subplot(211)
...
plt.imshow(speed_oversampl, aspect='auto', interpolation='none', cmap='jet')
# plt.colorbar()
plt.clim(0,1)
plt.subplot(212)
plt.plot(np.median(speed_oversampl,axis=0), label='median')
plt.plot(np.median(speed_oversampl,axis=0)-np.std(speed_oversampl,axis=0), 'C1', label='median-std')
plt.plot(np.median(speed_oversampl,axis=0)+np.std(speed_oversampl,axis=0), 'C1', label='median+std')
plt.axhline(np.median(speed_oversampl))
# plt.xlim(0, 178)
plt.legend()
plt.tight_layout()
    #%%
# =============================================================================
# Compute dFC speed average via window oversampling for one animal, with multiple windows length and tau
# - Resume this part to dfc_speed_oversampled_series
# =============================================================================
...
for idx_tt, tt in enumerate(time_windows_range):
    ...
    #Window oversampling by delta tau
...
plt.plot(time_windows_range,speed_windows_tau,'.-')#, label='%s'%tau_array)
plt.xlabel('W')
plt.ylabel('<v>')
...
height_stripe      = dfc_streamaux.shape[1]-windows_size-tau
speed_oversampl2    = np.array([dfc_speed(dfc_streamaux, vstep=windows_size + sp)[1][:height_stripe] for sp in tau_array])

plt.figure(7)
plt.clf()
# plt.violinplot(speed_oversampl2.T, showmeans=True)
plt.boxplot(speed_oversampl2.T, showmeans=True)
#%%
# =============================================================================
...
# start = time.time()
# for xx in ts2m:
#     aux_speed_median, aux_speed_dist  = dfc_speed_oversampled_series(xx, window_parameter, lag, get_speed_dist=True)
#     vel_list2m.append(aux_speed_dist)  
#     speed_median_2m.append(aux_speed_median)
# for xx in ts4m:
#     aux_speed_median, aux_speed_dist  = dfc_speed_oversampled_series(xx, window_parameter, lag, get_speed_dist=True)
...
# window_pooling_speed_wt2m     = window_pooling_speed(wt_index,  vel_list2m)
# window_pooling_speed_dki2m      = window_pooling_speed(dki_index,   vel_list2m)
# window_pooling_speed_wt4m     = window_pooling_speed(wt_index,  vel_list4m)
...
#     # Calculate distance of data points from mean in terms of standard deviation
...
#     # Identify outliers
#     outliers = data[prob < crit]
#     return outliers

# print(chauvenet_criterion(oip_wt),chauvenet_criterion(oip_dki))
# print(chauvenet_criterion(ro24_wt),chauvenet_criterion(ro24_dki))
...
# mask_chauvenet_oip_wtdki = np.array(chauvenet(oip_wt)) | np.array(chauvenet(oip_dki))
# mask_chauvenet_ro24_wtdki = np.array(chauvenet(ro24_dki)) | np.array(chauvenet(ro24_wt)) 
# mask_chauvenet_2m4m = np.array(chauvenet(delta_vel2m4m[dki_index])) | np.array(chauvenet(delta_vel2m4m[wt_index])) #), chauvenet(ro24_dki))
...
plt.subplot(121)
# plt.scatter(oip_wt, delta_vel2m4m[wt_index], label='wt slope = %s'%np.round(b_wt,3))
# plt.scatter(oip_dki, delta_vel2m4m[dki_index], label='dki slope = %s'%np.round(b_dki,3))
plt.scatter(delta_vel2m4m[wt_index], oip_wt, label='wt slope = %s'%np.round(b_wt,3))
plt.scatter(delta_vel2m4m[dki_index], oip_dki, label='dki slope = %s'%np.round(b_dki,3))
...
plt.xlabel(r'$\Delta <OiP_{4m-2m}>$')
plt.ylabel(r'$\Delta <v_{4m-2m}>$')
plt.legend()

plt.subplot(122)
# plt.scatter(ro24_wt, delta_vel2m4m[wt_index], label='wt slope = %s'%np.round(b_wt_ro,3))
# plt.scatter(ro24_dki, delta_vel2m4m[dki_index], label='dki slope = %s'%np.round(b_dki_ro,3))
plt.scatter(delta_vel2m4m[wt_index],ro24_wt, label='wt slope = %s'%np.round(b_wt_ro,3))
plt.scatter(delta_vel2m4m[dki_index],ro24_dki, label='dki slope = %s'%np.round(b_dki_ro,3))

plt.plot(xseq, a_wt_ro + b_wt_ro * xseq, color="C0")
plt.plot(xseq, a_dki_ro + b_dki_ro * xseq, color="C1")
plt.xlabel(r'$\Delta <RO24_{4m-2m}>$')
plt.ylabel(r'$\Delta <v_{4m-2m}>$')
plt.legend()
...
# plt.subplot(133)
# plt.title('%s %s'%(label_statistic[statistic], label_vel[idx_vel]))
# plt.scatter(oip_wt, ro24_wt, label='wt')
# plt.scatter(oip_dki, ro24_dki, label='dki')
# plt.ylabel(r'$\Delta <RO24_{4m-2m}>$')
# plt.xlabel(r'$\Delta <OiP_{4m-2m}>$')
# plt.legend()
#%%
