...
def build_df(cog, score_key):
    ...
    for age_key, age in [('2M','2M'), ('4M','4M')]:
        ...
        for i in range(y.size):
            if not np.isnan(y[i]):
                rows.append({'score': float(y[i]),
                             'Sex': sex[i], 'Genotype': geno[i], 'Age': age})
...
def _cell_median(df, sex, geno, age):
    v = df.loc[(df.Sex==sex)&(df.Genotype==geno)&(df.Age==age),'score'].values
...
def omnibus_permutation(df, effect='Sex', B=10000, rng=1):
    ...
    def med_and_n(df_):
        ...
        for s in levels['Sex']:
            for g in levels['Genotype']:
                for a in levels['Age']:
                    vals = df_.loc[(df_.Sex==s)&(df_.Genotype==g)&(df_.Age==a), 'score'].to_numpy()
    ...
    def contrast(df_):
        ...
        if effect == 'Sex:Age:Genotype':
            ...
            Fd4,Fd2,Md4,Md2 = ('Female','dKI','4M'),('Female','dKI','2M'),('Male','dKI','4M'),('Male','dKI','2M')
            Fw4,Fw2,Mw4,Mw2 = ('Female','WT','4M'), ('Female','WT','2M'), ('Male','WT','4M'), ('Male','WT','2M')
            val = ((med[Fd4]-med[Fd2]) - (med[Md4]-med[Md2])) - ((med[Fw4]-med[Fw2]) - (med[Mw4]-med[Mw2]))
            return val
    ...
    for _ in tqdm.tqdm(range(B), desc=f"Permutations for {effect}", ncols=80):
        ...
        if effect in ('Sex','Age','Genotype'):
            strata = {'Sex':['Genotype','Age'],
                      'Age':['Sex','Genotype'],
                      'Genotype':['Sex','Age']}[effect]
...
def holm_sidak(pvals):
    ...
    for k, idx in enumerate(order):
        ...
        adj[idx] = min(p[idx]* ( (1 - (1-0.05)) / alpha_k ), 1.0)  # display-friendly; decision uses step-down
    return adj
...
def planned_pairwise(df):
    ...
    out = pd.DataFrame(rows, columns=['Family','Stratum1','Stratum2','p','Cliffs_delta','HL_diff','HL_lo','HL_hi','n1','n2'])
    # Holm–Šidák within each family
    out['p_adj'] = out.groupby('Family')['p'].transform(lambda s: pd.Series(holm_sidak(s.values), index=s.index))
...
def analyze_task(cog, score_key, B=10000):
    ...
    # omnibus
    effects = ['Sex','Age','Genotype','Sex:Age','Sex:Genotype','Age:Genotype','Sex:Age:Genotype']
    om = []
    for eff in effects:
        print(f"Computing omnibus for effect {eff} ...")
        T, p = omnibus_permutation(df, effect=eff, B=B, rng=42)
        om.append([eff, T, p])
    omnibus_tbl = pd.DataFrame(om, columns=['Effect','Contrast_value','p_perm']).sort_values('Effect')
    # pairwise
...
print(om_OiP); print(pw_OiP); print(om_RO); print(pw_RO)
#%%
def save_stats(paths, score_key, B, omnibus_tbl, pairwise_tbl, rng=42):
    ...
    def row2dict(r):
        return {
            "family": r["Family"], "s1": r["Stratum1"], "s2": r["Stratum2"],
            "p": float(r["p"]), "p_adj": float(r["p_adj"]),
            "delta": float(r["Cliffs_delta"]),
            "HL": float(r["HL_diff"]), "lo": float(r["HL_lo"]), "hi": float(r["HL_hi"])
        }
    ...
    summary = {
        ...
        "seed": int(rng),
        "omnibus": [
            {"effect": r["Effect"], "contrast": float(r["Contrast_value"]), "p_perm": float(r["p_perm"])}
            for _, r in omnibus_tbl.iterrows()
        ],
        "pairwise_all": [row2dict(r) for _, r in pairwise_tbl.iterrows()],
        "pairwise_sig": [row2dict(r) for _, r in pairwise_tbl.iterrows() if r["p_adj"] < 0.05],
    }
...
def _p_to_stars(p):
    return "****" if p < 1e-4 else "***" if p < 1e-3 else "**" if p < 0.01 else "*" if p < 0.05 else ""

def _anchored_badge(ax, lines, colors=None, loc="upper left"):
    if not lines:
        return
    txt = "\n".join(lines)
    box = AnchoredText(txt, loc=2 if loc=="upper left" else 1, prop=dict(size=9),
                       frameon=True, bbox_to_anchor=None, bbox_transform=ax.transAxes)
...
def _draw_bracket(ax, x1, x2, y, stars, color="0.2", lw=1.2, frac=0.02, ylim=None):
    ...
    ax.plot([x1, x1, x2, x2], [y, y+0.02, y+0.02, y], color=color, lw=lw, clip_on=False)
    if stars and stars != "ns":
        ax.text((x1+x2)/2, y+0.024, stars, ha="center", va="bottom", fontsize=10, color=color)
    elif stars == "ns":
        ax.text((x1+x2)/2, y+0.024, "ns", ha="center", va="bottom", fontsize=9, color="0.5")


def _collect_sig(summary, family):
    return [r for r in summary["pairwise_sig"] if r["family"]==family]

def apply_stats_annotations(ax, summary, ylim=None,
                            show_families=("Age","Genotype","Sex"),
                            show_non_sig=True):
    if ylim is not None:
        ax.set_ylim(*ylim)

    # x positions in your plot
    age_map = {("WT","Male"):(1,2), ("dKI","Male"):(3,4),
               ("WT","Female"):(6,7), ("dKI","Female"):(8,9)}
    gen_map = {("Male","2M"):(1,3), ("Male","4M"):(2,4),
               ("Female","2M"):(6,8), ("Female","4M"):(7,9)}
    sex_map = {("WT","2M"):(1,6), ("WT","4M"):(2,7),
               ("dKI","2M"):(3,8), ("dKI","4M"):(4,9)}

    rows = summary.get("pairwise_all", summary.get("pairwise_sig", []))

    def stars(p):
        return "****" if p < 1e-4 else "***" if p < 1e-3 else "**" if p < 0.01 else "*" if p < 0.05 else ("ns" if show_non_sig else "")
...
def _add_omnibus_box(ax, summary, ypad=0.04):
    ...
    if not sig: return
    lines = [f"{o['effect']} Δ={o['contrast']:+.3f}, p_perm={_fmt_p(o['p_perm'])}" for o in sig]
    txt = "Omnibus: " + "; ".join(lines)
    ax.text(0.5, 1.02 + ypad, txt, transform=ax.transAxes,
...
def _add_pairwise_summary(ax, summary, family, color="k", where="top", max_items=4):
    ...
    if family == "Age":
        ...
        parts = [f"{r['s1']}-{r['s2']}: Δ={r['HL']:+.2f} [{r['lo']:+.2f},{r['hi']:+.2f}], p_adj={_fmt_p(r['p_adj'])}" for r in sig]
        label = "Age (4M−2M)"
    elif family == "Genotype":
        # items like ('Female','2M') or ('Male','4M')
        parts = [f"{r['s1']} {r['s2']}: Δ={r['HL']:+.2f}, p_adj={_fmt_p(r['p_adj'])}" for r in sig]
...
def _bracket(ax, x1, x2, y, text=None, color="k"):
    ...
    if text:
        ax.text((x1+x2)/2, y+0.025, text, ha="center", va="bottom", fontsize=9, color=color)
...
def annotate_panel_from_stats(ax, summary, panel="OiP", ylim=None):
    ...
    sig_age = {(r["s1"], r["s2"]) for r in summary["pairwise_sig"] if r["family"]=="Age" and r["p_adj"]<0.05}
    sig_gen = {(r["s1"], r["s2"]) for r in summary["pairwise_sig"] if r["family"]=="Genotype" and r["p_adj"]<0.05}

    # map helpers for brackets
    # Age family key: (Genotype, Sex)
    age_pos = {("WT","Male"):(1,2), ("dKI","Male"):(3,4),
               ("WT","Female"):(6,7), ("dKI","Female"):(8,9)}
    ...
    gen_pos = {("Male","2M"):(1,3), ("Male","4M"):(2,4),
               ("Female","2M"):(6,8), ("Female","4M"):(7,9)}
...
def _draw_bracket_axesy(ax, x1, x2, y_ax, stars, lw=1.3, color="0.2"):
    ...
    ax.plot([x1, x1, x2, x2], [y_ax, y_ax+0.02, y_ax+0.02, y_ax],
            transform=trans, color=color, lw=lw, clip_on=False)
    if stars:
        ax.text((x1+x2)/2, y_ax+0.025, stars if stars!="ns" else "ns",
                transform=trans, ha="center", va="bottom",
                fontsize=10 if stars!="ns" else 9, color=color)
...
def _main_effects_badge(ax, summary):
    ...
    have = {o["effect"]: o["p_perm"] for o in summary.get("omnibus", []) if o["effect"] in want}
    lines = []
...
def _plot_panel(ax, groups_male, groups_fem, title, ylabel, y0=0.0, ylim=None,
                jitter=0.2, seed=4, show_counts=False):
    ...
    # draw violins
    v1 = ax.violinplot(groups_male, positions=pos_m, showmeans=False, showmedians=False, showextrema=False)
    v2 = ax.violinplot(groups_fem,  positions=pos_f, showmeans=False, showmedians=False, showextrema=False)

    # colors
    col_m = 'C1'  # male
    col_f = 'C2'  # female
    for b in v1['bodies']:
        b.set_alpha(0.1); b.set_facecolor(col_m); b.set_edgecolor('black'); b.set_linewidth(0.8)
    for b in v2['bodies']:
        b.set_alpha(0.1); b.set_facecolor(col_f); b.set_edgecolor('black'); b.set_linewidth(0.8)
    ...
    def scatter_points(groups, positions, color):
        for g, x in zip(groups, positions):
            ...
            ax.scatter(xj, y, s=16, alpha=0.6, color=color, facecolors='none', linewidths=1, zorder=3)
    ...
    def annotate(groups, positions, color):
        for g, x in zip(groups, positions):
            ...
            if show_counts:
                n = int(np.sum(~np.isnan(g)))
                ax.text(x, ax.get_ylim()[0]-0.3 if ylim is None else ylim[0],
                        f"n={n}", ha='center', va='bottom', fontsize=9, color=color)
                # n label at bottom
    ...
    ax.set_xticklabels(['WT 2M','WT 4M','dKI 2M','dKI 4M']*2, rotation=0, fontsize=12)

    # headers for sex blocks
    topy = (ax.get_ylim()[1] if ylim is None else ylim[1])
    trans = mtransforms.blended_transform_factory(ax.transData, ax.transAxes)
    ax.text(pos_m.mean(), 0.09, 'Male', transform=trans,
            ha='center', va='bottom', fontsize=13, weight='bold', fontstyle='italic', zorder=6, color='C1')
    ax.text(pos_f.mean(), 0.09, 'Female', transform=trans,
            ha='center', va='bottom', fontsize=13, weight='bold', fontstyle='italic', zorder=6, color='C2')
...
_plot_panel(ax1, m_OiP, f_OiP, title="OiP", ylabel="OiP score", y0=0.0, show_counts=False)
...
_plot_panel(ax2, m_RO, f_RO,  title="RO24h", ylabel="RO24h score", y0=0.0, show_counts=False)
...
plt.tight_layout()
plt.savefig(paths["f_cog"] / f"violin_OiP_RO24h_{timecourse_folder}.png", dpi=300, bbox_inches='tight')
plt.savefig(paths["f_cog"] / f"violin_OiP_RO24h_{timecourse_folder}.svg", dpi=300, bbox_inches='tight')
plt.show()
