#!/usr/bin/env python3
"""Modern preprocessing entrypoint for meta-connectivity cognitive data.

This script consolidates the legacy `metaconnectivity/old_useful/cognitive_data_ts_sorted.py`
and the newer `allegiance/src/prep_cog_groups.py` into a single, reusable module.
It intersects 2m and 4m time-series recordings, harmonises cognitive metadata,
builds group masks, and writes the standardised preprocessing bundle consumed by
downstream meta-connectivity and allegiance pipelines.
"""
...
class GroupingPayload:
    """Grouping masks and lookup tables generated during preprocessing."""
...
@dataclass
class PrepResult:
    """Outputs generated by :func:`prepare_cognitive_dataset`."""
...
def _group_indices(df: pd.DataFrame, columns: Sequence[str]) -> Dict[Tuple[str, ...], np.ndarray]:
    """Return a mapping from column-value tuples to dataframe indices."""
...
def prepare_cognitive_dataset(
    *,
    ...
    # Keep only subjects with intersected functional data and valid timecourses.
    cog_data_filtered = (
    ...
    metadata = {
        ...
        "anat_labels": np.asarray(anat_labels, dtype=object),
        "transient": np.array(transient, dtype=np.int32),
        "dataset_name": np.asarray(dataset_name),
        "timecourse_folder": np.asarray(timecourse_folder),
    }
