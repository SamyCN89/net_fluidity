...
mem_spec = ['d HIP', 'v HIP', 'd DG', 'v DG', 'PERI', 'ENT', 'SUB', 'ReRh',
            'THAL memory', 5, 6, 7, 8, 9, 10, 11, 13, 14]
...
plt.yticks(np.arange(len(plot_order_reversed)) * offset,
           [anat_labels[i] for i in plot_order_reversed])

# --- Color the ytick labels to match their network ---
ax = plt.gca()
for ticklabel, ridx in zip(ax.get_yticklabels(), plot_order_reversed):
