...
ts_bin = np.array(
    [np.where(ts[i] > np.std(ts[i]) / 10, 1, 0) for i in range(n_animals)]
)


# Plot the ts of one animal in a matrix
logger.info("Starting compute_allegiance_local visualizations")
...
for i in range(dfc_stream.shape[1]):
    # Compute the allegiance communities for each time window
    dfc_communities[i], sort_allegiance[i], contingency_matrix[i] = (
        fun_allegiance_communities(
            dfc_stream[0, i],
