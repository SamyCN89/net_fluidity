...
filenames = {
    period: filename_sort_mat(os.path.join(paths["timeseries"], folder))
    for period, folder in folders.items()
}
hash_numbers = {period: extract_hash_numbers(filenames[period]) for period in filenames}
common_ids, intind_2m, intind_4m = np.intersect1d(
    hash_numbers["2mois"], hash_numbers["4mois"], return_indices=True
)
...
cog_data_df["ro24h_4m-2m"] = (
    cog_data_df.loc[:, "RO24h_4M"] - cog_data_df.loc[:, "RO24h_2M"]
)
cog_data_df["ro24h_4m+2m"] = (
    cog_data_df.loc[:, "RO24h_4M"] + cog_data_df.loc[:, "RO24h_2M"]
)
...
cog_data_filtered = cog_data_filtered[
    (cog_data_filtered["TC_2M"] == 0) & (cog_data_filtered["TC_4M"] == 0)
].copy()
...
index_tsintcog = np.array((intind_2m, intind_4m))[
    :, inter_cogfun[1]
]  # intersection of 2m,4m and coginfo
...
cog_data_filtered["group_NOR"] = (
    cog_data_filtered["Genotype"].astype(str)
    + "_"
    + cog_data_filtered["Sexe"].astype(str)
    + "_"
...
cog_data_filtered.to_csv(
    paths["preprocessed"] / "cog_data_sorted_2m4m.csv", index=False
)
