...
for animal in range(n_animals):
    print(f"Animal {animal} - Min: {min_modules[animal]}, Max: {max_modules[animal]}, Mean: {mean_modules[animal]}")
...
def build_xy_for_factor(labels_i, masks_i, mean_modules):
    ...
    # Parse base name + age
    parsed = []
    for lbl, m in zip(labels_i, masks_i):
        lbl = str(lbl)
        if lbl.endswith("2m"):
            base = lbl[:-2].strip()
            age = "2m"
    ...
    return base_names, np.array(x_means), np.array(y_means), np.array(x_err), np.array(y_err)

# ---- Plot: one panel per factor ----
n_factors = len(label_variables)
fig, axes = plt.subplots(1, n_factors, figsize=(6 * n_factors, 6), sharex=True, sharey=True)
if n_factors == 1:
    axes = [axes]

factors_labels = ('oip', 'nor', 'genotype', 'sex')

for ax, (labels_i, masks_i), fi in zip(axes, zip(label_variables, mask_groups), range(n_factors)):
    base_names, x_mu, y_mu, x_se, y_se = build_xy_for_factor(labels_i, masks_i, mean_modules)

    # Scatter with error bars
    ax.errorbar(x_mu, y_mu, xerr=x_se, yerr=y_se, fmt='o', capsize=3, lw=1, alpha=0.9)

    # Annotate each point
    for bn, x, y in zip(base_names, x_mu, y_mu):
...
# plot the dfc_communities_sorted matrix of 1st animal
plot_matrix(dfc_communities_sorted[0].T, "dFC Communities - Animal 0", cmap="viridis", save=False)
# %%
...
def cohesion_probability(communities):
    ...
    for reg1 in range(n_regions):
        for reg2 in range(reg1+1, n_regions):
            aux1 = communities[:,reg1]
            aux2 = communities[:,reg2]
            cohesion_timeseries = (aux1- aux2)
            # count the number of time windows where the two regions are in the same module
            cohesion_probability[reg1, reg2] = (cohesion_timeseries!=0).sum() / len(cohesion_timeseries)

            # print(cohesion_probability[reg1, reg2])
    return cohesion_probability


cohesion_probability_all = np.zeros((n_animals, n_regions, n_regions))

for animal in range(n_animals):
    cohesion_probability_all[animal] = cohesion_probability(dfc_communities_sorted[animal])

#%%
#plot imshow of cohesion_probability
plt.figure(figsize=(10, 8))
plt.imshow(cohesion_probability_all[5], aspect="auto", interpolation="none", cmap="viridis",
           vmin=0, vmax=1)
plt.colorbar()
plt.title("Cohesion Probability")
plt.xlabel("Region 2")
plt.ylabel("Region 1")
plt.xticks(np.arange(n_regions), labels=anat_labels[sort_allegiances[0, 0].astype(int)], rotation=90)
...
for i in range(n_animals):
    ...
    plt.imshow(cohesion_probability_all[i], aspect="auto", interpolation="none", cmap="viridis",
               vmin=0, vmax=1)
    plt.xticks([])
