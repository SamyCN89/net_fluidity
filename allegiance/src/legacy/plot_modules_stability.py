...
# %% Compute the number of modules per time window
...
def build_xy_for_factor(labels_i, masks_i, mean_modules):
    ...
    # Parse base name + age
    parsed = []
    # for each label and mask, extract base and age
    for lbl, m in zip(labels_i, masks_i, strict=False):
        lbl = str(lbl) #label of group
        # Check if the label ends with a recognized age
        if lbl.endswith("2m"):
            base = lbl[:-2].strip()
            age = "2m"
