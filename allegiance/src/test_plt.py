...
dfc_communities_sorted = np.take_along_axis(
    dfc_communities, sort_allegiances.astype(int), axis=-1
)
...
plt.imshow(
    contingency_matrices[0, 0, sort_allegiances[0, 0].astype(int)][
...
# %%
# Aggregation matrix
cont_mat_n_pairs_agg = np.stack(
    [
        (np.sum(contingency_matrices[ind], axis=0) / n_windows)
        for ind in range(n_animals)
    ]
)
...
ind_grp_sort = np.concatenate(
    [np.squeeze(np.argwhere(grp == True)) for grp in mask_groups[2]]
)
# sort the cont_mat_n_pairs_agg_fcd by ind_grp_sort
cont_mat_n_pairs_agg_fcd_sortgrp = cont_mat_n_pairs_agg_fcd[ind_grp_sort, :][
    :, ind_grp_sort
]
...
plt.imshow(
    cont_mat_n_pairs_agg_fcd_sortgrp,
    aspect="auto",
    interpolation="none",
    cmap="viridis",
)
...
agreement = np.stack(
    [
        build_agreement_matrix_vectorized(dfc_communities_sorted[indv])
        for indv in tqdm(range(n_animals))
    ]
)
# %%
plt.figure(figsize=(10, 8))
