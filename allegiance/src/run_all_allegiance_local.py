...
parser.add_argument(
    "--n_jobs", type=int, default=4, help="Number of parallel jobs to run (default: 4)"
)
parser.add_argument(
    "--env",
    type=str,
...
parser.add_argument(
    "--window_size", type=int, default=9, help="DFC window size (default: 9)"
)
parser.add_argument(
    "--lag", type=int, default=1, help="Lag between windows (default: 1)"
)
parser.add_argument(
    "--timecourse_folder",
    type=str,
...
parser.add_argument(
    "--n_runs", type=int, default=1000, help="Number of modularity runs (default: 1000)"
)
parser.add_argument(
    "--gamma",
    type=float,
    default=100,
    help="Gamma parameter for modularity (default: 100)",
)
...
dfc_stream = np.transpose(
    dfc_data["dfc_stream"], (0, 3, 2, 1)
)  # (n_animals, n_windows, n_regions, n_regions)
...
def run_one_job(animal_idx, window_idx):
    out_file = (
        out_dir / f"{filename_dfc}_animal_{animal_idx:02d}_window_{window_idx:04d}.npz"
    )
    if out_file.exists():
...
Parallel(n_jobs=processors, backend="loky")(
    delayed(run_one_job)(a, w) for a in range(n_animals) for w in range(n_windows)
)
